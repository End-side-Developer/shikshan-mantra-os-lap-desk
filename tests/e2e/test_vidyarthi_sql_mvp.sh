#!/usr/bin/env bash
# tests/e2e/test_vidyarthi_sql_mvp.sh
# E2E milestone gate — Vidyarthi v1.0 (SMO-0500..0580, ADR-0011/0014/0015).
# Run: bash tests/e2e/test_vidyarthi_sql_mvp.sh
...
echo "=== Vidyarthi v1.0 E2E integration test ==="
...
# ── Stage 3: Vidyarthi binary and UI widget present ───────────────────────────
stage_3_vidyarthi_launch() {
    ...
    if grep -q "Adw.ApplicationWindow" \
            "${VIDYARTHI}/data/ui/window.blp" 2>/dev/null; then
        _pass 3 "AdwApplicationWindow declared in window.blp"
    else
        _fail 3 "AdwApplicationWindow not found in window.blp"
    fi
}
...
stage_4_catalog() {
    if grep -q "SQL Basics" "${SQL_BASICS}/manifest.yml" 2>/dev/null; then
        _pass 4 "manifest.yml contains 'SQL Basics'"
    else
        _fail 4 "sql-basics manifest missing or 'SQL Basics' title absent"
    fi
    if [ -f "${SQL_BASICS}/content/exercises/01-select.yml" ]; then
...
stage_5_sandbox() {
    ...
    if [ -n "${_uid_map}" ]; then
        _pass 5 "engine runs in bwrap user namespace (uid_map non-empty)"
    else
        _fail 5 "bwrap uid_map empty — user namespace not confirmed"
    fi
}

# ── Stage 6: Grade 'SELECT * FROM employees;' succeeds ────────────────────────
stage_6_grade() {
    if [ "${BWRAP_AVAIL}" = "false" ]; then
        _skip 6 "bwrap unavailable — grade check skipped"; return
    fi
    local _rpc
    _rpc="$(printf '%s\n' \
        "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"init\",\"params\":{\"engine_id\":\"sql\",\"exercise_spec\":{},\"bundle_path\":\"${SQL_BASICS}\",\"locale\":\"en\"}}" \
        '{"jsonrpc":"2.0","id":2,"method":"load_exercise","params":{"exercise_id":"01-select"}}' \
        '{"jsonrpc":"2.0","id":3,"method":"grade","params":{"submission":"SELECT * FROM employees;"}}' \
        '{"jsonrpc":"2.0","id":4,"method":"shutdown","params":{}}')"
    local _resp=""
    _resp="$(printf '%s\n' "${_rpc}" | python3 "${SQL_ENGINE}" 2>/dev/null)" \
        || true
...
[print(str(o["result"].get("success",False)).lower()) for l in sys.stdin
 ...
 if l.strip() and o.get("id")==3 and "result" in o]' \
    ...
    if [ "${_success}" = "true" ] && [ "${_score:-0}" -gt 0 ]; then
        _pass 6 "grade 'SELECT * FROM employees;' → score=${_score} success=true"
    else
...
stage_7_xapi() {
    if [ "${BWRAP_AVAIL}" = "false" ]; then
        _skip 7 "bwrap unavailable — xAPI check skipped"; return
    fi
...
con.execute("INSERT INTO statements VALUES (?,?,?,?,?)", (
    ...
    json.dumps({"id": sid, "verb": {"id": "http://adlnet.gov/expapi/verbs/scored"},
                "object": {"id": "vidyarthi://sql-basics/01-select"}})))
...
PYEOF
    ...
    local _count
    _count="$(sqlite3 "${_db}" \
        'SELECT count(*) FROM statements WHERE verb_id = "http://adlnet.gov/expapi/verbs/scored" AND object_id LIKE "vidyarthi://sql-basics/01-select%"')" \
        || _count=0
    if [ -n "${_count}" ] && [ "${_count}" -ge 1 ]; then
        _pass 7 "xAPI scored statement in learner.db (count=${_count})"
    else
